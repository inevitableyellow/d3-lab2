...
function bilink(root) {
    ...
    for (const d of root.leaves()) for (const [source, target, airline] of d.outgoing) {
...
function createChart(data) {
    ...
    const link = svg.append("g")
        ...
        .attr("stroke", ([,, airline]) => airlineColor[airline] || colornone)  // ADD
        .attr("d", ([i, o]) => line(i.path(o)))
        .each(function(d) { d.link = this; });

    function overed(event, d) {
            link.style("mix-blend-mode", null);
            link.attr("stroke", colornone);  // dim all
            d3.select(this).attr("font-weight", "bold");
            d3.selectAll(d.incoming.map(d => d.link))
                .attr("stroke", ([,, airline]) => airlineColor[airline] || colornone).raise();
            d3.selectAll(d.incoming.map(([d]) => d.text)).attr("fill", colorin).attr("font-weight", "bold");
            d3.selectAll(d.outgoing.map(d => d.link))
                .attr("stroke", ([,, airline]) => airlineColor[airline] || colornone).raise();
